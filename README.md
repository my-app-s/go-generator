# HTML Generator in Go

![status: dev](https://img.shields.io/badge/status-dev-orange)
![Go Version](https://img.shields.io/badge/go-1.25%2B-blue.svg)
![License](https://img.shields.io/badge/license-GNU%20AGPLv3-red.svg)

Простой и быстрый генератор статических страниц, написанный на Go.

## 🚀 Demo
Сгенерированные страницы можно посмотреть здесь: 
[[Demo ссылка на GitHub Pages](https://my-app-s.github.io/go-generator/)]

## ⚠️ Disclaimer
Код в данном репозитории предоставляется на условиях **"AS IS"** (как есть). Автор не несет ответственности за любые последствия использования данных материалов.

> **Используйте на свой страх и риск.**

## 📜 License
Проект распространяется под лицензией **GNU AGPLv3**. Подробности в файле LICENSE.
